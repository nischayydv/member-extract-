...
<body>
    <div class="container">
        ...
        <div class="info-banner">
            <h3>ðŸ›¡ï¸ Safe & Secure Member Adding</h3>
            <p>
                âœ… This bot follows Telegram's guidelines to protect your account<br>
                âš ï¸ Member adding is intentionally slow (4-10 seconds delay) to prevent bans<br>
                ðŸ” Uses device spoofing and smart flood protection for maximum safety<br>
                ðŸ’¡ Your account safety is our priority - patience ensures success!
            </p>
        </div>
        ...
        <div class="logs-container">
            <div class="logs-header">
                <span>ðŸ“Š Live Activity Logs</span>
        ...
        <div class="footer">
            <p><strong>ðŸ” Security Notice:</strong> This is your private dashboard. Do not share this URL.</p>
...
async def invite_task(user_id, update: Update, context: ContextTypes.DEFAULT_TYPE = None):
    try:
        ...
        if not data:
            await update.message.reply_text("âŒ No configuration found. Use /run first.")
            return
        ...
        if device_info:
            log_to_user(user_id, 'INFO', f"âœ“ Using device: {device_info['device_model']}")
        ...
        try:
            ...
            if not await client.is_user_authorized():
                await update.message.reply_text("âŒ Session expired. Please use /run to login again.")
            ...
            await send_chat_log(update, f"âœ… Logged in as: {me.first_name} (@{me.username or 'N/A'})", context)
        except Exception as e:
            logger.error(f"âŒ Login failed: {e}")
            await update.message.reply_text(f"âŒ Login failed: {e}")
            if str(user_id) in ACTIVE_TASKS:
                del ACTIVE_TASKS[str(user_id)]
            await client.disconnect()
            return
        ...
        while ACTIVE_TASKS.get(str(user_id), {}).get('running', False):
            try:
                ...
                log_to_user(user_id, 'INFO', f"ðŸš€ Task started | {len(participants)} members found")
                await send_chat_log(update, 
                    f"ðŸš€ Task Started Successfully!\n\n"
                    f"ðŸ“Š Members Found: {len(participants)}\n"
                    f"â± Delay: {min_delay}-{max_delay}s (Safe Mode)\n"
                    f"ðŸ“± Device: {device_info['device_model'] if device_info else 'Default'}\n\n"
                    f"ðŸ’¡ Use /pause, /resume, or /stop to control", context)
                ...
                for user in participants:
                    ...
                    if invited_ok:
                        log_to_user(user_id, 'INFO', f"âœ… [INVITED] {first_name} (@{username if username else uid})")
                        if member_count % 5 == 0:
                            await send_chat_log(update, 
                                f"âœ… Progress Update:\n"
                    ...
                    if info and info.startswith('floodwait'):
                        try:
                            ...
                            await send_chat_log(update, f"âš ï¸ FloodWait! Waiting {wait_time}s...", context)
                    ...
                    if info == 'peerflood':
                        ...
                        await send_chat_log(update, f"âš ï¸ PeerFlood! Pausing {pause_time//60} minutes for safety...", context)
                        await asyncio.sleep(pause_time)
                        continue
                    ...
                    if not settings['skip_dm_on_fail'] and uid not in already_sent:
                        try:
                            ...
                            log_to_user(user_id, 'INFO', f"ðŸ“¨ [DM SENT] {first_name}")
                            ...
                            log_to_user(user_id, 'ERROR', f"âŒ DM failed: {type(e).__name__}")
                ...
                log_to_user(user_id, 'INFO', "âœ“ Batch completed. Restarting in 30s...")
                await send_chat_log(update, "âœ… Batch completed. Restarting in 30 seconds...", context)
                await asyncio.sleep(30)

            except Exception as e:
                logger.error(f"âŒ Loop error: {e}")
                log_to_user(user_id, 'ERROR', f"âŒ Error: {type(e).__name__}")
                await send_chat_log(update, f"âŒ Error: {type(e).__name__}\nRetrying in 60s...", context)
        ...
        final_msg = (
            f"ðŸŽ‰ Task Completed!\n\n"
            f"âœ… Invited: {ACTIVE_TASKS[str(user_id)]['invited_count']}\n"
            f"ðŸ“¨ DMs: {ACTIVE_TASKS[str(user_id)]['dm_count']}\n"
            f"âŒ Failed: {ACTIVE_TASKS[str(user_id)]['failed_count']}\n"
            f"â± Time: {int(elapsed//60)}m {int(elapsed%60)}s\n\n"
            f"âš¡ Bot by @NY_BOTS"
        )
        await send_chat_log(update, final_msg, context)
        log_to_user(user_id, 'INFO', "ðŸŽ‰ Task completed successfully!")
        ...
        logger.error(f"âŒ Task error: {e}")
        if str(user_id) in ACTIVE_TASKS:
            del ACTIVE_TASKS[str(user_id)]
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton('ðŸš€ Start New Task', callback_data='start_task')],
        [InlineKeyboardButton('ðŸ”„ Restart Last Task', callback_data='rerun_task')],
        [InlineKeyboardButton('ðŸ“Š View Statistics', callback_data='view_stats')],
        [InlineKeyboardButton('ðŸŒ Open Dashboard', url=dashboard_url)],
        [InlineKeyboardButton('â“ Help', callback_data='show_help')]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    welcome_text = (
        "ðŸ¤– <b>Welcome to Telegram Invite Bot!</b>\n\n"
        "âœ¨ <b>Premium Features:</b>\n"
        "â€¢ ðŸ” Advanced device spoofing\n"
        "â€¢ ðŸ’¾ Persistent session storage\n"
        "â€¢ ðŸ›¡ï¸ Smart flood protection\n"
        "â€¢ ðŸ“Š Real-time dashboard\n"
        "â€¢ ðŸŽ¯ Intelligent member targeting\n"
        "â€¢ ðŸ”’ Account safety guaranteed\n\n"
        "âš ï¸ <b>Important:</b> Member adding is intentionally slow (4-10s delay) "
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    help_text = (
        ...
        "ðŸš€ /run - Start new task\n"
        "ðŸ”„ /rerun - Repeat last task\n"
        "â¸ /pause - Pause running task\n"
        "â–¶ï¸ /resume - Resume paused task\n"
        "â¹ /stop - Stop task completely\n"
        "ðŸ“Š /stats - View statistics\n"
        "ðŸ—‘ /clear - Clear history files\n"
        "ðŸ”„ /reset - Reset session\n"
        "âŒ /cancel - Cancel operation\n\n"
        "âš ï¸ <b>Important Notes:</b>\n"
        "â€¢ Wait 10-15 minutes between login attempts\n"
        "â€¢ NEVER share OTP codes with anyone\n"
        "â€¢ Use /reset if you get security errors\n"
        "â€¢ OTP format: You can enter spaces (e.g., '1 2 3 4 5')\n"
        "â€¢ Slow speed = Safe account (follows Telegram rules)\n\n"
        "ðŸ’¡ <b>Tip:</b> View live dashboard from the main menu!\n\n"
...
async def stats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    stats_text = (
        ...
        f"âœ… Total Invited: <b>{stats['total_invited']}</b>\n"
        f"ðŸ“¨ Total DMs: <b>{stats['total_dms_sent']}</b>\n"
        f"âŒ Total Failed: <b>{stats['total_failed']}</b>\n"
        f"ðŸ”„ Total Runs: <b>{stats['total_runs']}</b>\n"
        f"ðŸ”´ Status: <b>{'ðŸŸ¢ Running' if active else 'âš« Idle'}</b>"
    )
    ...
    if active:
        ...
        stats_text += (
            f"\n\nðŸ”¥ <b>Current Task:</b>\n"
            f"âœ… Invited: <b>{task['invited_count']}</b>\n"
            f"ðŸ“¨ DMs: <b>{task['dm_count']}</b>\n"
            f"âŒ Failed: <b>{task['failed_count']}</b>\n"
            f"â± Runtime: <b>{runtime//60}m {runtime%60}s</b>\n"
...
async def pause_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in ACTIVE_TASKS:
        await update.message.reply_text("âŒ No active task to pause.")
        return
...
async def resume_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in ACTIVE_TASKS:
        await update.message.reply_text("âŒ No active task to resume.")
        return
    
    ACTIVE_TASKS[user_id]['paused'] = False
    log_to_user(user_id, 'INFO', "â–¶ï¸ Task resumed by user")
    await update.message.reply_text("â–¶ï¸ <b>Task Resumed</b>", parse_mode='HTML')

async def stop_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = str(update.effective_user.id)
    
    if user_id not in ACTIVE_TASKS:
        await update.message.reply_text("âŒ No active task to stop.")
        return
    
    ACTIVE_TASKS[user_id]['running'] = False
    log_to_user(user_id, 'WARNING', "â¹ Task stopped by user")
    await update.message.reply_text("â¹ <b>Task Stopping...</b> Please wait.", parse_mode='HTML')
...
async def clear_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(f"ðŸ—‘ User {user_id} cleared {cleared} files")
    await update.message.reply_text(
        f"ðŸ—‘ <b>Cleared {cleared} history files</b>\n\nâœ… Fresh start ready!",
        parse_mode='HTML'
    )
...
async def reset_session(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in ACTIVE_TASKS:
        await update.message.reply_text("âŒ Stop task first with /stop")
        return
    ...
    if os.path.exists(f'{session_name}.session'):
        try:
            ...
            removed = True
            logger.info(f"ðŸ”„ Session reset for user {user_id}")
        except Exception as e:
            logger.error(f"Error: {e}")
    
    if removed:
        await update.message.reply_text(
            "ðŸ”„ <b>Session Reset Complete!</b>\n\n"
            "âš ï¸ <b>IMPORTANT:</b> Wait 10-15 minutes before using /run again.\n"
            "This prevents Telegram security blocks.\n\n"
            "âœ… You can login fresh after the wait period.",
            parse_mode='HTML'
        )
    else:
        await update.message.reply_text("â„¹ï¸ No session file found to reset")
...
async def run_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in ACTIVE_TASKS:
        await message.reply_text("âŒ Task already running! Use /stop first")
    ...
    if user_id in USER_HISTORY:
        ...
        if os.path.exists(f'{session_name}.session'):
            try:
                ...
                await message.reply_text("ðŸ” Checking saved session...")
                ...
                if await client.is_user_authorized():
                    ...
                    logger.info(f"âœ“ Using saved session for {me.first_name}")
                    await message.reply_text(
                        f"âœ… <b>Using Saved Session!</b>\n\n"
                        f"ðŸ‘¤ Logged in as: <b>{me.first_name}</b>\n"
                        f"ðŸš€ Starting task now...\n\n"
                    ...
                    await client.disconnect()
                    os.remove(f'{session_name}.session')
                    logger.warning(f"âš  Session expired for user {user_id}")
                    await message.reply_text("âš ï¸ Session expired. Starting new login...")
            except Exception as e:
                ...
                try:
                    ...
                    pass
                await message.reply_text("âš ï¸ Session error. Starting fresh login...")
    
    await message.reply_text(
        "ðŸš€ <b>Step 1/7: API Credentials</b>\n\n"
        "ðŸ“ Get your credentials from:\n"
        "https://my.telegram.org\n\n"
        "ðŸ”¢ Enter your <b>API_ID:</b>",
        parse_mode='HTML'
    )
    return API_ID
...
async def api_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        logger.info(f"âœ“ API ID received from user {update.effective_user.id}")
        await update.message.reply_text(
            "âœ… API ID saved!\n\nðŸ”‘ <b>Step 2/7: API Hash</b>\n\nEnter your <b>API_HASH:</b>",
            parse_mode='HTML'
        )
        return API_HASH
    except ValueError:
        await update.message.reply_text("âŒ Invalid API ID. Must be numbers only.\n\nTry again:")
        return API_ID

async def api_hash(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data['api_hash'] = update.message.text.strip()
    logger.info(f"âœ“ API Hash received from user {update.effective_user.id}")
    await update.message.reply_text(
        "âœ… API Hash saved!\n\n"
        "ðŸ“± <b>Step 3/7: Phone Number</b>\n\n"
...
async def phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not phone_number.startswith('+'):
        await update.message.reply_text("âŒ Phone must start with +\n\nTry again:")
        return PHONE
    ...
    try:
        await update.message.reply_text(
            "ðŸ”Œ <b>Connecting to Telegram...</b>\n\n"
            f"ðŸ“± Device: <code>{device_info['device_model']}</code>\n"
            f"ðŸ–¥ System: <code>{device_info['device_string']}</code>\n"
            f"ðŸ“¦ App: <code>{device_info['app_version']}</code>\n\n"
            "â³ Please wait...",
            parse_mode='HTML'
        )
        ...
        logger.info(f"âœ“ OTP sent to {phone_number}")
        await update.message.reply_text(
            "âœ… <b>OTP Code Sent!</b>\n\n"
            "ðŸ“± <b>Step 4/7: Verification Code</b>\n\n"
            "Check your Telegram app for a 5-digit code.\n\n"
            "ðŸ”¢ Enter the code (spaces are OK):\n"
            "Examples:\n"
            "â€¢ 12345\n"
            "â€¢ 1 2 3 4 5\n\n"
            "âš ï¸ <b>NEVER share this code with anyone!</b>\n"
            "ðŸ”’ This message will be auto-deleted for security.",
            parse_mode='HTML'
        )
        return OTP_CODE
    except Exception as e:
        logger.error(f"âŒ Error sending OTP: {e}")
        await update.message.reply_text(
            f"âŒ <b>Error:</b> {e}\n\n"
...
async def otp_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not otp or len(otp) != 5:
        await context.bot.send_message(
            ...
            text=f"âŒ Invalid OTP format. Must be 5 digits.\n\nYou entered: {len(otp)} digits\n\nTry again:"
        )
        return OTP_CODE
    ...
    if user_id not in TEMP_CLIENTS:
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text="âŒ Session expired. Please use /run to start over."
        )
        return ConversationHandler.END
    ...
    try:
        ...
        if await client.is_user_authorized():
            ...
            await context.bot.send_message(
                chat_id=update.effective_chat.id,
                text=f"ðŸŽ‰ <b>Login Successful!</b>\n\n"
                     f"ðŸ‘¤ Account: <b>{me.first_name}</b>\n"
                     f"ðŸ“± Username: <b>@{me.username or 'N/A'}</b>\n"
                     f"ðŸ’¾ Session saved for future use!\n\n"
                     f"ðŸ“ <b>Step 5/7: Source Group</b>\n\n"
                     f"Enter the source group to scrape members from:\n\n"
                     f"Examples:\n"
                     f"â€¢ @groupusername\n"
                     f"â€¢ https://t.me/groupname\n"
                     f"â€¢ -100123456789 (group ID)",
                parse_mode='HTML'
            )
            ...
            await context.bot.send_message(
                chat_id=update.effective_chat.id,
                text="âŒ Login failed. Authorization not granted.\n\nUse /run to try again."
            )
            if user_id in TEMP_CLIENTS:
                try:
                    await TEMP_CLIENTS[user_id]['client'].disconnect()
        ...
        logger.info(f"ðŸ” 2FA required for user {user_id}")
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text="ðŸ” <b>Step 4.5/7: Two-Factor Authentication</b>\n\n"
                 "Your account has 2FA enabled.\n\n"
                 "ðŸ”‘ Enter your 2FA password:",
            parse_mode='HTML'
        )
        return TWO_FA_PASSWORD
    except Exception as e:
        logger.error(f"âŒ OTP verification failed: {e}")
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text=f"âŒ <b>Error:</b> {str(e)}\n\n"
                 "Possible reasons:\n"
                 "â€¢ Wrong OTP code\n"
                 "â€¢ Code expired (request new one)\n"
                 "â€¢ Security block (wait 10-15 min)\n\n"
                 "Use /run to try again.",
            parse_mode='HTML'
        )
        if user_id in TEMP_CLIENTS:
...
async def two_fa_password(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in TEMP_CLIENTS:
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text="âŒ Session expired. Use /run to start over."
        )
        return ConversationHandler.END
    ...
    try:
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text="ðŸ” Verifying 2FA password..."
        )
        ...
        if await client.is_user_authorized():
            ...
            await context.bot.send_message(
                chat_id=update.effective_chat.id,
                text="âŒ 2FA verification failed.\n\nUse /run to try again."
            )
            if user_id in TEMP_CLIENTS:
                try:
                    await TEMP_CLIENTS[user_id]['client'].disconnect()
...
async def source_group(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(f"âœ“ Source group set for user {update.effective_user.id}")
    await update.message.reply_text(
        "âœ… Source group saved!\n\n"
        "ðŸŽ¯ <b>Step 6/7: Target Group</b>\n\n"
        "Enter the target group to add members to:\n\n"
        "Examples:\n"
        "â€¢ @targetgroup\n"
        "â€¢ https://t.me/targetgroup\n"
        "â€¢ -100987654321",
        parse_mode='HTML'
    )
    return TARGET
...
async def target_group(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(f"âœ“ Target group set for user {update.effective_user.id}")
    await update.message.reply_text(
        "âœ… Target group saved!\n\n"
        "ðŸ”— <b>Step 7/7: Invite Link</b>\n\n"
...
async def invite_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    save_user_history()
    logger.info(f"âœ… Setup complete for user {user_id}")
    ...
    await update.message.reply_text(
        "ðŸŽ‰ <b>Setup Complete!</b>\n\n"
        "âœ… All configurations saved\n"
        "ðŸš€ Starting task now...\n\n"
        "ðŸŒ <b>Live Dashboard:</b>\n"
        f"<code>{dashboard_url}</code>\n\n"
        "ðŸ’¡ You can control the task with:\n"
        "â¸ /pause - Pause task\n"
        "â–¶ï¸ /resume - Resume task\n"
        "â¹ /stop - Stop task\n"
        "ðŸ“Š /stats - View progress\n\n"
        "âš¡ <i>Bot by</i> <a href='https://t.me/NY_BOTS'>@NY_BOTS</a>",
        parse_mode='HTML',
        disable_web_page_preview=True
    )
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(f"âŒ Setup cancelled by user {user_id}")
    await update.message.reply_text(
        "âŒ <b>Setup Cancelled</b>\n\n"
...
async def rerun_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in USER_HISTORY:
        await message.reply_text("âŒ No previous configuration found.\n\nUse /run to setup first.")
        return
    
    session_name = f'session_{user_id}'
    if not os.path.exists(f'{session_name}.session'):
        await message.reply_text("âŒ Session expired. Use /run to login again.")
        return
    
    logger.info(f"ðŸ”„ Rerun requested by user {user_id}")
    await message.reply_text("ðŸ”„ <b>Restarting previous task...</b>", parse_mode='HTML')
...
def main():
    ...
    logger.info(f"ðŸŒ Dashboard available at: http://0.0.0.0:{PORT}")
